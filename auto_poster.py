...
def create_text_thumbnail(text, filename_prefix):
    try:
        ...
        if not lines: lines = ["가상화폐(코인) 무료 정보방 모집", "핵심 실무 가이드"]
...
def generate_post(campaign, keyword):
    ...
    draft_prompt = f"""당신은 15년 차 도산전문 법률사무소 수석 법률상담관입니다.
...
{{
  ...
  "thumb_hook": "{keyword}\\n핵심 실무 가이드 (줄바꿈 \\n)",
  "vibe_keywords": "finance balance law",
...
---METADATA_END---"""
    ...
    vibe_keywords = meta.get('vibe_keywords', 'finance balance law')
